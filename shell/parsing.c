...
{
	if (arg[0] == '$') {
		if (strcmp(arg, "$?") == 0) {  // variable magica
			sprintf(arg, "%d", status);
			return arg;
		}

		char *value = getenv(arg + 1);
		if (!value || strlen(value) == 0) {  // no existe
			return NULL;
		}

		size_t value_size = strlen(value);
		if (value_size >
		    strlen(arg)) {  // Veo que pasa con el tamaño de arg
			arg = realloc(arg, 1 + value_size);
		}

		return strcpy(arg, value);
	}
	return arg;
}
