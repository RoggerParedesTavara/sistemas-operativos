...
sys_exofork(void)
{
	...
	struct Env *env;
	int e;
	if ((e = env_alloc(&env, curenv->env_id)) < 0) {
		return e;
	}
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if (page_insert(env->env_pgdir, page, va, perm | PTE_P | PTE_U) < 0) {
		page_free(page);
		return -E_NO_MEM;
	}
...
sys_page_map(envid_t srcenvid, void *srcva, envid_t dstenvid, void *dstva, int perm)
{
	...
	if (page_insert(dstenv->env_pgdir, page, dstva, perm | PTE_U | PTE_P) < 0) {
		return -E_NO_MEM;
	}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if (va < UTOP) {
		if (!(va % PGSIZE == 0) || !(perm & PTE_SYSCALL))
			return -E_INVAL;
