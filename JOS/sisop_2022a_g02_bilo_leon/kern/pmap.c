...
boot_alloc(uint32_t n)
{
	...
	int pages_exceeded = npages_basemem - (int) (n + nextfree) / PGSIZE;
	if (pages_exceeded > 0) {
		result = nextfree;
		nextfree = ROUNDUP((char *) (nextfree + n), PGSIZE);
		return result;
	} else {
		panic("[boot_alloc]: Not sufficient memory");
	}
...
mem_init(void)
{
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	boot_map_region(kern_pgdir, KERNBASE, (KERNTOP - KERNBASE), 0, PTE_W);
...
page_init(void)
{
	...
	// free pages!
	size_t i;
	int mem_use = PADDR(boot_alloc(0));
	for (i = 0; i < npages; i++) {
		if ((i == 0) ||
		    ((i >= (IOPHYSMEM / PGSIZE)) &&
		     (i < ((EXTPHYSMEM + mem_use) / PGSIZE))) ||
...
{
	pde_t *result_pgdir = pgdir + PDX(va);
	if (!create && !*result_pgdir) {
		return NULL;
	} else if (!*result_pgdir && create) {
		struct PageInfo *pp = page_alloc(ALLOC_ZERO);
		if (!pp) {
			return NULL;
		}
		pp->pp_ref++;
		*result_pgdir = page2pa(pp) | PTE_P | PTE_U | PTE_W;
	}
	return (pte_t *) KADDR(PTE_ADDR(*result_pgdir)) + PTX(va);
}


static void
map_normal_page(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	for (int i = 0; i < size / PGSIZE; i++) {
		pte_t *pgtable =
		        pgdir_walk(pgdir, (const void *) (va + i * PGSIZE), 1);
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// Fill this function in
#ifndef TP1_PSE
	map_normal_page(pgdir, va, size, pa, perm);
#else
	size_t aux = 0;
	while (aux < size) {
		if (((va % PTSIZE == 0) && (pa % PTSIZE == 0)) ||
			...
			*p = pa | perm | PTE_P | PTE_PS;
			va += PTSIZE;
			pa += PTSIZE;
			aux += PTSIZE;
		} else {
			pte_t *pgtable = pgdir_walk(pgdir, (const void *) va, 1);
...
page_remove(pde_t *pgdir, void *va)
{
	...
	if (pp) {
		page_decref(pp);
		// busco la pg table correspondiente a la va:
		pte_t *pgtable = pgdir_walk(pgdir, va, 0);
		*pgtable = 0;
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	//
	// Your code here:
	size_t aux = ROUNDUP(size, PGSIZE);
	if (base + aux > MMIOLIM) {
		panic("[mmio_map_region]: base+size is bigger than MMIOLIM");
	}
	boot_map_region(kern_pgdir, base, aux, pa, PTE_W | PTE_PCD | PTE_PWT);
	base += aux;
	return (void *) base - aux;
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (; start < end; start++) {
		if ((uintptr_t) start >= ULIM) {
			user_mem_check_addr =
			        (uintptr_t)(start < va ? va : start);
			return -E_FAULT;
		}
		pte_t *pgtable = pgdir_walk(env->env_pgdir, start, 0);
		if (!pgtable || ((*pgtable & (perm | PTE_P)) != (perm | PTE_P))) {
			user_mem_check_addr =
			        (uintptr_t)(start < va ? va : start);
			return -E_FAULT;
		}
